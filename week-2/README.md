# Week 2 - JavaScript Practice & Node.js Basics
...
*   **`examSimulator.js`**: A simulator for exam conditions, grading, and testing scenarios.
*   **`libraryManagement.js`**: Implements a library system using classes to manage books, borrowing, returning, and filtering available items.
*   **`moviePlatform.js`**: Simulates logic for a movie platform or catalog management.
*   **`onlineCourses.js`**: Models an online course platform and enrollment mechanics.
*   **`shoppingCart.js`**: Logic for a simple shopping cart system, calculating totals and managing items.
*   **`studentMarks.js` / `studentPerformance.js`**: Scripts to calculate, analyze, and manage student performance, grades, and metrics.
*   **`sumUsingSpreadOperator.js`**: Examples of using the ES6 spread/rest operators for mathematical operations and array manipulations.
*   **`tempAnalyzer.js`**: A utility for analyzing temperature data and calculating averages or extremes.
*   **`transactionAnalyzer.js`**: Analyzes and processes mock financial transactions or banking activities.
...
node <filename>.js
```
*Example:* `node libraryManagement.js`

For projects inside the directories (`Backend-1`, etc.), navigate into the folder, install dependencies if necessary, and start the application:
```bash
cd Backend-1
npm install
node server.js
